...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    <aside className="sidebar">
      <a className="brand" href="/"><span className="brand-mark"><Layers2 size={24} /></span><span>表里<small>SheetDelta</small></span></a>
      <nav aria-label="主导航"><span className="nav-group-label">工作台</span><a className="nav-item active" href="/" aria-current="page"><ArrowLeftRight size={18} />表格对比</a><button className={`nav-item ${showRules ? 'selected' : ''}`} onClick={() => setShowRules(!showRules)}><FolderOpen size={18} />我的规则<span className="nav-count">{rules.length}</span></button><span className="nav-group-label resources-label">使用与接入</span><a className="nav-item" href="/guide/"><BookOpen size={18} />使用指南</a><a className="nav-item" href="/docs/"><Code2 size={18} />开发者文档</a></nav>
    ...
    <main id="workspace"><p className="sr-only" role="status">{busy ? '正在比较表格' : result ? `比较完成：${result.summary.changed} 行修改，${result.summary.added} 行新增，${result.summary.removed} 行删除。` : ''}</p>
      ...
      <footer className="page-footer"><span>表里 SheetDelta <span className="footer-dot">·</span> 让数据变化有迹可循</span><div><a href="/guide/">使用指南</a><a href="/docs/">npm 接入文档<ArrowRight size={13} /></a></div></footer>
    </main></div>
  </div>;
}
